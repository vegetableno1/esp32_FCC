...
#define SAMPLE_DEVICE_NAME          "NingBo_FaXiang"
...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0xFF, 0x00,
        /* device name */
        0x0f, 0x09, 'N', 'I', 'N', 'G', 'B', 'O', '_', 'F', 'A', 'X', 'I','A', 'N', 'G'
};
        ...
        case ESP_GATTS_WRITE_EVT://GATT写事件，手机给开发板的发送数据，不需要回复
            if (!param->write.is_prep){
                ...
                if (heart_rate_handle_table[IDX_CHAR_CFG_A] == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){//进水命令
                        ESP_LOGI(GATTS_TABLE_TAG, "notify enable");
                        uint8_t notify_data[] = "th_data";
...
void ble_app(void)
{
    ...
    ret = esp_bt_controller_init(&bt_cfg);//初始化蓝牙控制器，此函数只能被调用一次，且必须在其他蓝牙功能被调用之前调用
    if (ret) {
        ESP_LOGE(GATTS_TABLE_TAG, "%s enable controller failed: %s", __func__, esp_err_to_name(ret));
        return;
    }

    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);//如果想要动态改变蓝牙模式不能直接调用该函数，先disable关闭蓝牙再使用该API来改变蓝牙模式
    if (ret) {
        ESP_LOGE(GATTS_TABLE_TAG, "%s enable controller failed: %s", __func__, esp_err_to_name(ret));
        return;
    }

    ret = esp_bluedroid_init();//蓝牙栈 `bluedroid stack` 包括了BT和 BLE 使用的基本的define和API
    if (ret) {
    ...
    ret = esp_ble_gatts_register_callback(gatts_event_handler);//注册 GATT 回调函数，回调函数具体内容会在下文说明
    if (ret){
        ESP_LOGE(GATTS_TABLE_TAG, "gatts register error, error code = %x", ret);
        return;
    }

    ret = esp_ble_gap_register_callback(gap_event_handler);//注册 GAP 回调函数
    if (ret){
